...
class montecarlo:
    ...
    def execute(self, ref_value_index = 0):
            print('MonteCarlo() has been executed, this may take some time depending on the time sequence and number of simulations defined.')
            print('NOTE: Use data with reasonable standard deviation in order to prevent exponentional growth of the function that cannot be plotted properly, recognize such abnormal values by a + sign anywhere in the data executed below.\nThe model that will be able to handle big standard deviations is currently being worked on, thank you for your patience.')
            import pandas as pd
