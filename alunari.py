...
class montecarlo:
    ...
    def execute(self, ref_value_index = 0):
            ...
            for num_sim in range(self.num_sims): 
                count = 0
                daily_volatility = percent_change.std()

                index_array = []
                simulated_index = today_value * (1 + np.random.normal(0, daily_volatility))
                ...
                for num_day in range(self.time_seq):
                    if count == self.time_seq:
                        break
                    simulated_index = index_array[count] * (1 + np.random.normal(0, daily_volatility))
                    index_array.append(simulated_index)
                    count += 1
